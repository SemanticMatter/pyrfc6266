...
import pyrfc6266._main as _main
from pyrfc6266._main import ContentDisposition

# Tests sourced from http://test.greenbytes.de/tech/tc2231/


def test_greenbytes_inlonly():
    assert _main.parse("inline") == ("inline", [])


def test_greenbytes_inlonlyquoted():
    with pytest.raises(_main.ParseException):
        _main.parse('"inline"')


def test_greenbytes_inlwithasciifilename():
    s = 'inline; filename="foo.html"'
    assert _main.parse(s) == (
        "inline",
        [
            ContentDisposition("filename", "foo.html"),
        ],
    )
    assert _main.parse_filename(s) == "foo.html"


def test_greenbytes_inlwithfnattach():
    assert _main.parse('inline; filename="Not an attachment!"') == (
        "inline",
        [
...
def test_greenbytes_inlwithasciifilenamepdf():
    assert _main.parse('inline; filename="foo.pdf"') == (
        "inline",
        [
...
def test_greenbytes_attonly():
    assert _main.parse("attachment") == ("attachment", [])


def test_greenbytes_attonlyquoted():
    with pytest.raises(_main.ParseException):
        _main.parse('"attachment"')
...
def test_greenbytes_attwithasciifilename25():
    ...
    assert _main.parse_filename(s) == "0000000000111111111122222"
...
def test_greenbytes_attwithasciifilename35():
    ...
    assert _main.parse_filename(s) == "00000000001111111111222222222233333"
...
def test_greenbytes_attwithasciifnescapedquote():
    ...
    assert _main.parse_filename(s) == '"quoting" tested.html'
...
def test_greenbytes_attwithquotedsemicolon():
    ...
    assert _main.parse_filename(s) == "Here's a semicolon;.html"
...
def test_greenbytes_attwithtokfncommanq():
    with pytest.raises(_main.ParseException):
        _main.parse(r"""attachment; filename=foo,bar.html""")
...
def test_greenbytes_attemptyparam():
    with pytest.raises(_main.ParseException):
        _main.parse(r"""attachment; ;filename=foo""")
...
def test_greenbytes_attwithfntokensq():
    ...
    assert _main.parse_filename(s) == "'foo.bar'"
...
def test_greenbytes_attwithisofnplain():
    ...
    assert _main.parse_filename(s) == "foo-ä.html"
...
def test_greenbytes_attwithutf8fnplain():
    ...
    assert _main.parse_filename(s) == "foo-Ã¤.html"
...
def test_greenbytes_attwithfnrawpctenca():
    ...
    assert _main.parse_filename(s) == "foo-%41.html"
...
def test_greenbytes_attwithfnusingpct():
    ...
    assert _main.parse_filename(s) == "50%.html"
...
def test_greenbytes_attwithfilenamepctandiso():
    ...
    assert _main.parse_filename(s) == "ä-%41.html"
...
def test_greenbytes_attwithfnrawpctenclong():
    ...
    assert _main.parse_filename(s) == "foo-%c3%a4-%e2%82%ac.html"
...
def test_greenbytes_attwith2filenames():
    with pytest.raises(_main.ParseException):
        _main.parse(r'''attachment; filename="foo.html"; filename="bar.html"''')


def test_greenbytes_attfnbrokentoken():
    with pytest.raises(_main.ParseException):
        _main.parse(r"""attachment; filename=foo[1](2).html""")


def test_greenbytes_attfnbrokentokeniso():
    with pytest.raises(_main.ParseException):
        _main.parse(r"""attachment; filename=foo-ä.html""")


def test_greenbytes_attfnbrokentokenutf():
    with pytest.raises(_main.ParseException):
        _main.parse(r"""attachment; filename=foo-Ã¤.html""")


def test_greenbytes_attmissingdisposition():
    with pytest.raises(_main.ParseException):
        _main.parse(r"""filename=foo.html""")


def test_greenbytes_attmissingdisposition2():
    with pytest.raises(_main.ParseException):
        _main.parse(r"""x=y; filename=foo.html""")


def test_greenbytes_attmissingdisposition3():
    with pytest.raises(_main.ParseException):
        _main.parse(r""""foo; filename=bar;baz"; filename=qux""")


def test_greenbytes_attmissingdisposition4():
    with pytest.raises(_main.ParseException):
        _main.parse(r"""filename=foo.html, filename=bar.html""")
...
def test_greenbytes_doublecolon():
    with pytest.raises(_main.ParseException):
        _main.parse(r""": inline; attachment; filename=foo.html""")
...
def test_greenbytes_attandinline2():
    with pytest.raises(_main.ParseException):
        _main.parse(r"""attachment; inline; filename=foo.html""")


def test_greenbytes_attbrokenquotedfn():
    with pytest.raises(_main.ParseException):
        _main.parse(r"""attachment; filename="foo.html".txt""")


def test_greenbytes_attbrokenquotedfn2():
    with pytest.raises(_main.ParseException):
        _main.parse(r"""attachment; filename="bar""")


def test_greenbytes_attbrokenquotedfn3():
    with pytest.raises(_main.ParseException):
        _main.parse(r"""attachment; filename=foo"bar;baz"qux""")


def test_greenbytes_attmultinstances():
    with pytest.raises(_main.ParseException):
        _main.parse(r"""attachment; filename=foo.html, attachment; filename=bar.html""")


def test_greenbytes_attmissingdelim():
    with pytest.raises(_main.ParseException):
        _main.parse(r"""attachment; foo=foo filename=bar""")


def test_greenbytes_attmissingdelim2():
    with pytest.raises(_main.ParseException):
        _main.parse(r"""attachment; filename=bar foo=foo """)
...
def test_greenbytes_attreversed():
    with pytest.raises(_main.ParseException):
        _main.parse(r"""filename=foo.html; attachment""")
...
def test_greenbytes_attconfusedparam():
    ...
    assert _main.parse_filename(s) is None
...
def test_greenbytes_attabspath():
    ...
    assert _main.parse_filename(s) == "_foo.html"
...
def test_greenbytes_dispext():
    assert _main.parse(r"""foobar""") == ("foobar", [])


def test_greenbytes_dispextbadfn():
    assert _main.parse(r'''attachment; example="filename=example.txt"''') == (
        "attachment",
        [
...
def test_greenbytes_attwithfn2231noc():
    assert _main.parse(r"""attachment; filename*=''foo-%c3%a4-%e2%82%ac.html""") == (
        "attachment",
        [],
    )
...
def test_greenbytes_attwithfn2231utf8comp():
    ...
    assert _main.parse_filename(s) == "foo-ä.html"


def test_greenbytes_attwithfn2231utf8_bad():
    with pytest.raises(_main.ParseException):
        _main.parse(r"""attachment; filename*=iso-8859-1''foo-%c3%a4-%e2%82%ac.html""")


def test_greenbytes_attwithfn2231iso_bad():
    with pytest.raises(_main.ParseException):
        _main.parse(r"""attachment; filename*=utf-8''foo-%E4.html""")


def test_greenbytes_attwithfn2231ws1():
    with pytest.raises(_main.ParseException):
        _main.parse(r"""attachment; filename *=UTF-8''foo-%c3%a4.html""")
...
def test_greenbytes_attwithfn2231quot2():
    with pytest.raises(_main.ParseException):
        _main.parse(r'''attachment; filename*="foo%20bar.html"''')
...
def test_greenbytes_attwithfn2231nbadpct1():
    with pytest.raises(_main.ParseException):
        _main.parse(r"""attachment; filename*=UTF-8''foo%""")


def test_greenbytes_attwithfn2231nbadpct2():
    with pytest.raises(_main.ParseException):
        _main.parse(r"""attachment; filename*=UTF-8''f%oo.html""")
...
def test_greenbytes_attwithfn2231dpct():
    ...
    assert _main.parse_filename(s) == "A-%41.html"
...
def test_greenbytes_attfncontqs():
    ...
    assert _main.parse_filename(s) == "foobar.html"
...
def test_greenbytes_attfncontqs2():
    ...
    assert _main.parse_filename(s) == "fooban.html"
...
def test_greenbytes_attfncontlz():
    ...
    assert _main.parse_filename(s) == "foo"
...
def test_greenbytes_attfncontord():
    ...
    assert _main.parse_filename(s) == "foobar"
...
def test_greenbytes_attrfc2047token():
    with pytest.raises(_main.ParseException):
        _main.parse(r"""attachment; filename==?ISO-8859-1?Q?foo-=E4.html?=""")


def test_greenbytes_attrfc2047quoted():
    assert _main.parse(r'''attachment; filename="=?ISO-8859-1?Q?foo-=E4.html?="''') == (
        "attachment",
        [
...
def test_requests_from_header():
    ...
    assert _main.requests_response_to_filename(response) == "different_file.txt"


def test_requests_from_url():
    response = requests.Response()
    response.url = "https://example.com/path/a_file.txt?token=123"
    assert _main.requests_response_to_filename(response) == "a_file.txt"


def test_requests_undiscoverable():
    response = requests.Response()
    response.url = "https://example.com/"
    assert _main.requests_response_to_filename(response).startswith("unknown-")


def test_fix_issue_001_known_disposition():
    s = r'''attachment;filename*="utf-8' '100MB.zip"'''
    assert _main.parse_filename(s) == "100MB.zip"


def test_fix_issue_001_unknown_disposition():
    s = r'''atachment;filename*="utf-8' '100MB.zip"'''
    assert _main.parse_filename(s, enforce_content_disposition_type=True) is None
